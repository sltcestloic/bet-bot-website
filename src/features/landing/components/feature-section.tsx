...
import type { LandingFeature } from '../config/landing-features'
    ...
    <section id={feature.id} className={`feature-band text-white ${index % 2 === 0 ? 'bg-[#1b1e26]' : 'bg-[#20232c]'}`}>
      <div className={`mx-auto grid max-w-[1180px] items-center gap-12 px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-2 lg:gap-20 ${reversed ? 'lg:[&>*:first-child]:order-2' : ''}`}>
        <div data-aos="fade-up" className="relative flex min-h-[410px] items-center justify-center sm:min-h-[560px]">
          ...
          <div className="discord-window relative z-10 w-full max-w-[520px] overflow-hidden rounded-md border border-[#474a55] bg-[#313338] shadow-[0_28px_70px_rgba(25,27,34,0.22)]">
            <div className="flex h-11 items-center justify-between border-b border-white/8 bg-[#292b31] px-4 text-[11px] font-bold text-white/55">
              <span className="flex items-center gap-2"><span className="text-white/35">#</span> pronostics</span>
              <span className="flex gap-1"><i className="size-1.5 rounded-full bg-white/30" /><i className="size-1.5 rounded-full bg-white/30" /><i className="size-1.5 rounded-full bg-white/30" /></span>
            </div>
          ...
          <div className="absolute bottom-[4%] right-[2%] z-20 hidden items-center gap-2 rounded-md border border-white/10 bg-[#11131a] px-3 py-2 text-xs font-extrabold text-white shadow-lg sm:flex">
        ...
        <div data-aos="fade-up" data-aos-delay="50" className="max-w-[520px]">
          ...
          <h2 className="text-3xl font-black leading-[1.08] tracking-[0] text-white text-balance sm:text-5xl">{feature.title}</h2>
          ...
          <ul className="mt-7 grid gap-3 text-sm font-bold text-[#eceef5]">
            {feature.bullets.map((bullet) => (
              <li key={bullet} className="flex items-center gap-3"><span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-[#5865f2]/20 text-[#9ca5ff]"><Check className="size-3.5" /></span>{bullet}</li>
            ))}
          </ul>
        </div>
      </div>
