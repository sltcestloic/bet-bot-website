...
export function Hero({ viewer }: { viewer?: LandingViewer | null }) {
  return (
    <section id="home" className="hero relative isolate min-h-[92svh] overflow-hidden bg-[#11131a] text-white">
      ...
      <div className="mx-auto grid min-h-[92svh] max-w-[1240px] items-center gap-8 px-5 pt-24 pb-8 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-6 lg:pt-28 lg:pb-12">
        <div className="relative z-10 max-w-[650px] self-center lg:pb-10">
          <div className="mb-5 inline-flex items-center gap-2 text-xs font-bold tracking-[0.14em] text-[#cbd0ff] uppercase">
          ...
          <h1 className="max-w-[680px] text-[clamp(2.2rem,4.35vw,4rem)] leading-[1.02] font-black tracking-[0] text-balance">
          ...
          <p className="mt-6 max-w-[590px] text-base leading-7 text-[#d9dce6] sm:text-lg sm:leading-8">
            Sport, esport, pièces fictives et classements : Bet Bot donne à chaque match une raison de revenir, de jouer et de chambrer
            entre amis.
          </p>
          ...
          <div className="mt-7 hidden flex-wrap gap-x-6 gap-y-2 text-xs font-semibold text-white/70 sm:flex">
            <span className="inline-flex items-center gap-2">
              <Check className="size-4 text-[#66d9a8]" /> Pièces 100 % fictives
            </span>
            <span className="inline-flex items-center gap-2">
              <Check className="size-4 text-[#66d9a8]" /> Installation rapide
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="hero-ui hero-ui-result absolute right-[1%] bottom-[1%] z-10 w-[52%] max-w-[385px] overflow-hidden rounded-md border border-white/10 bg-[#313338] shadow-[0_30px_90px_rgba(0,0,0,0.68)] sm:right-[2%] sm:bottom-[3%] lg:right-[1%] lg:bottom-[5%] lg:w-[56%]">
          ...
          <div className="absolute bottom-[7%] left-[10%] z-30 hidden rounded-md border border-[#f4c25b]/30 bg-[#1d2028]/90 px-4 py-3 shadow-xl backdrop-blur sm:block lg:bottom-[10%] lg:left-[1%]">
            <p className="text-[10px] font-bold tracking-[0.12em] text-white/50 uppercase">Cote du combiné</p>
            <p className="mt-1 flex items-center gap-2 text-xl font-black text-white">
              <Sparkles className="size-4 text-[#f4c25b]" /> 6,35
            </p>
          </div>
        </div>
      </div>
      <div className="absolute right-5 bottom-3 text-[9px] text-white/35 sm:right-8">Photo : Josemar Duarte / Unsplash</div>
    </section>
  )
}
