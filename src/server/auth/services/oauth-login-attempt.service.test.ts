...
import { type OAuthLoginAttemptEntity } from '@/server/auth/entities/oauth-login-attempt.entity'
...
describe('OAuthLoginAttemptService', () => {
  ...
  it('creates a short-lived login attempt without storing raw tokens', async () => {
    const service = new OAuthLoginAttemptService(repository as unknown as Repository<OAuthLoginAttemptEntity>, tokenService, () => now)
